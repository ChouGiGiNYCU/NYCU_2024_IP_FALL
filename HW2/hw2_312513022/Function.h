...
YCbCr RGB2YCbCr(const unsigned char R,const unsigned char G,const unsigned char B);
RGB YCbCr2RGC(const unsigned char Y,const unsigned char Cb,const unsigned char Cr);
...
YCbCr RGB2YCbCr(const unsigned char R,const unsigned char G,const unsigned char B){
    ...
    int Cr =  0.439*R - 0.368*G - 0.071*B + 128;

    // 確保計算結果在0到255範圍內
    Y  = fmax(0,fmin(Y,255));
    Cb = fmax(0,fmin(Cb,255));
    Cr = fmax(0,fmin(Cr,255));

    YCbCr tmp = {(unsigned char)Y, (unsigned char)Cb, (unsigned char)Cr};
    return tmp;
}

RGB YCbCr2RGB(const unsigned char y,const unsigned char cb,const unsigned char cr){
    double Y = (double) y;
    double Cb = (double) cb;
    double Cr = (double) cr;
    ...
    g = fmax(0, fmin(255, g));
    b = fmax(0, fmin(255, b));
    RGB tmp = {(unsigned char)r,(unsigned char)g,(unsigned char)b};
    return tmp;
}
